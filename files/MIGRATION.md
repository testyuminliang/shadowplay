...
> 最近更新：2026-06-24 — v13 纹理贴图渲染器（甲片改"贴图+裁剪+高光"，真实感大幅提升）。
...
### v9 — 时序平滑消抖 + Smooth 滑块（2026-06-24）
...
### v10 — 掌心/手背朝向判定（重做 v3，更稳）（2026-06-24）
用户反馈：**手掌正面朝镜头、手指伸直**时，甲片还是会显示（指甲在手背那面，本不该出现）。这正是 v3 删掉全局掌心检测后留下的回归。

- **判定信号（关键）：** 用手掌大三角 `(wrist=0, 食指MCP=5, 小指MCP=17)` 的法线 **z 分量**，它正好等于三点在图像平面的 **2D 绕向** `ax*by - ay*bx`——**完全不用 MediaPipe 的噪声 z**，且三角形大、数值稳。这就是为什么 v4 的逐指法线失败、而手掌法线（v3 方向）能成。
- **左右手 / 镜像不变性：** 绕向符号的「掌心 or 手背」含义随手性翻转；可证明 **(handedness 标签, 绕向符号) 这个组合对图像镜像不变**（镜像同时翻转标签和符号），所以无论摄像头流是否镜像，规则都成立。规则：`palm = (label=='Right') ? nz>0 : nz<0`。
- **防边界闪烁：** `|nz|` 很小（手侧立、朝向模糊）时**不判掌心**，交给逐指 fade 处理；nz 还经 One Euro 平滑。
- **Palm 开关（默认开）+ Debug HUD：** 控制栏加 Palm 按钮可一键开关此功能；Debug 模式在手腕旁打印 `label / PALM→hide|BACK→show / nz`，万一符号判反一眼可见、改一个常量即可。
- **无头浏览器验证：** 注入四种 (label, nz符号) 组合 + 开关：`Right&nz>0→隐藏(0px)`、`Left&nz>0→显示(250px)`、`Right&nz<0→显示`、`Left&nz<0→隐藏`、关掉开关→显示。全部符合预期。
- **与 v3 的区别：** v3 是全局隐藏、太粗暴（弯指/猫爪也被隐藏）；本版判定更稳。
- **遗留（v11 解决）：** v10 是「掌心朝镜头就整手隐藏」，导致**猫爪姿势**（掌心朝镜头但手指卷曲、指甲其实可见）也被隐藏 → 见 v11。

### v11 — 掌心判定 × 逐指卷曲结合（2026-06-24）
用户反馈：开 Palm 时**猫爪姿势识别不到**；关 Palm 时**平摊手掌又会显示甲片**——要求把两者**结合**。

- **核心模型：** 指甲是否可见 = 末节指节背面（指甲面）是否朝镜头，由两因素共同决定，**逐指**判断：
  1. **卷向镜头（猫爪）优先：** `zDelta = tip.z − dip.z < −0.04`（指尖比 DIP 更近镜头）→ 指甲转入视野 → **显示**，**无视掌心/手背**。这就是让猫爪在掌心朝镜头时仍能显示的「覆盖」规则。
  2. **否则（手指伸直）才看朝向：** 掌心朝镜头 → 指甲在另一面 → 隐藏；手背朝镜头 → 显示，并按 edge-on fade 渐隐（保留 v8 拇指侧棱隐藏）。
- **效果：** 平摊手掌正面=隐藏、猫爪=显示、手背正常=显示，三者兼顾。
- **取舍：** 把 v10 的「整手 `if(palmFacing) return`」改成逐指；卷曲阈值 `CURL_TOWARD=0.04` 是经验值，可调（猫爪没显示就调小，平摊手掌偶尔漏出就调大）。仍用平滑后的 `zDelta`，减少抖动误判。
- **无头浏览器验证：** 注入四种姿势（Palm 开）：平摊掌心→0px(隐藏)、猫爪(zDelta=−0.06)→72px(显示)、手背伸直→72px(显示)、手背卷离→0px(隐藏)，全部符合预期。
- **遗留（v12 解决）：** 这里的「卷曲」用的是 `zDelta`（MediaPipe 噪声 z），真机上猫爪常常触发不了，导致掌心朝镜头时猫爪仍被隐藏。

### v12 — 卷曲判定改用 2D 夹角（去掉噪声 z）（2026-06-24）
用户反馈：v11 的猫爪在真机上还是出不来（"有 palm 的时候蜷缩指甲也要有甲片"）。

- **根因：** v11 用 `zDelta = tip.z - dip.z` 判断卷曲，依赖 MediaPipe 的相对深度 z——噪声大、不可靠（正是 v4 的老教训）。真手卷曲时 z 信号不稳，判不出"卷曲"，于是落回掌心隐藏。
- **改法：** 卷曲改用**纯 2D 夹角**——近端段 `(MCP→PIP)` 与末端段 `(DIP→TIP)` 的 cos：伸直≈+1，卷过 ~90° 趋于/低于 0。完全不碰 z，鲁棒。`curled = cos < CURL_COS(默认0.4)`，且对 cos 做了平滑。
- **可见性模型（异或）：** 指甲朝镜头 ⟺ `palmFacing === curled`：
  - 伸直 → 可见 ⟺ 手背朝镜头；
  - 卷曲(末节翻转~180°) → 可见 ⟺ 手掌朝镜头（猫爪）。
  - 结果：平摊手掌隐藏、猫爪显示、手背显示、握拳隐藏，全部来自**无深度**信号。
- **edge-on fade** 只在"手背+伸直"的正常视角生效（保留 v8 拇指侧棱隐藏）；猫爪/卷曲按全强度画。
- **无头验证：** 用真正 2D 卷曲的 landmark（不依赖 z）：平摊掌心→0、猫爪→121px(显示)、手背伸直→250px、握拳→0，全部符合预期。
- **可调：** `CURL_COS`（猫爪不灵敏→调小；平摊手掌偶尔漏→调大）。

### v13 — 纹理贴图渲染器（真实感）（2026-06-24，当前版本）
用户反馈：甲片"非常失真"，想放进图里那种好看的真实美甲（蓝格纹、酒红金箔等）。

- **根因：** 之前是**纯矢量填充**（贝塞尔形状灌纯色/简单渐变 + 两片高光椭圆）= 像贴纸，没有材质，做不出图案/釉面/金箔。
- **改法（纹理贴图，选定的 Path 1）：** `drawSingleNail` 从"画形状填色"改为"**贴图**"：
  1. **软投影**（先用甲形带 shadowBlur 填一层）→ 甲片"长"在指甲上、不悬浮；
  2. **clip 裁剪**到贝塞尔甲形，`drawImage(设计纹理)` 填入；
  3. 顶部叠**釉面高光**（左上→淡出的线性渐变）+ **镜面高光条** + **甲根阴影**；
  4. 甲缘描一圈**亮边**。
- **设计纹理（程序化、自包含）：** `makeTexture()` 把每个设计画到离屏 canvas（朝向：顶=指尖），含 8 款：Navy / Navy 格纹 / 奶油碎花 / 酒红 / 酒红金箔 / 法式 / 丁香紫 / 粉蓝渐变。绘制原语：`glaze/vgrad/diag/plaid/floral/glitter/frenchTip`。素材库 swatch 直接用纹理的 `toDataURL()` 当预览，所见即所得。
- **关键：可扩展到真实/AI 素材：** 真实单片 PNG 或 **Imagen 生成的甲片**只要塞进 `design.tex`（一个 `<img>`/canvas）就能走同一套贴图管线——这就把"中间过渡档（PNG 仿射贴图）"和后续 AI 生成链路接上了。
- **无头验证（截图肉眼确认）：** 注入手部、分别选 Navy 格纹 / 酒红金箔 / 法式渲染并合成到肤色底上：格纹清晰落在甲形内、金箔有闪粉颗粒+镜面高光、整体有釉面反光与软投影，真实感明显提升（截图见 scratchpad，非交付物）。
- **后续可做：** 透视 4 角变形（现为仿射，曲面更贴合需 WebGL）；甲形随设计微调；接 AI 生成的单片 PNG。
...
### ✅ 掌心方向处理（v10 已修复）
- 之前不区分掌心/手背，掌心正对镜头、手指伸直时甲片会错误显示在掌侧
- **v10 用手掌法线的 2D 绕向 + handedness 判定掌心/手背**（详见迭代历史 v10），掌心朝镜头时隐藏甲片
- 提供 Palm 开关（默认开）+ Debug HUD 显示判定，符号若与实机相反可据此翻常量
- **v11 已结合逐指卷曲：** 掌心朝镜头但卷指（猫爪、指甲可见）→ 显示；平摊手掌正面 → 隐藏；手背 → 显示
